...
- **[SPEC.md](./SPEC.md)** — 仕様書。何ができるアプリなのか、機能・ショートカット・
  データの保存場所・上限値。専門知識のない読み手を想定して書いてある。
  「この機能はどういう挙動が正しいのか」を確かめるときはここを見る。
- **CLAUDE.md（このファイル）** — 実装するときの決まりごと。とくに
  **踏むと分からないまま壊れる落とし穴**を集めてある。コードを書く前に読む。

以前あった `SYSTEM_DESIGN.md` は、実装が進むにつれ内容が実態と食い違ったため削除した
（Zustand・ブックマークフォルダ・単一 WebView など、採用されなかった設計が残っていた）。
設計の意図は上記 2 つに集約している。
