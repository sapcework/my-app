...
| 復元 | JSON バックアップファイルから全ストアを上書き復元。**Supabase側への反映が成功した場合のみ**ローカルにも反映する（クラウドを正とするため、失敗時はローカルを変更せずエラートーストを表示） |
...
アプリ起動時の4桁PIN入力画面と、テンキーUI本体。`usePasscodeStore`の`verify()`が5回連続失敗で30秒ロックを返すため、ロック中は`PinPad`を表示せず「◯秒後に再試行できます」というカウントダウン表示に切り替える。
