...
## Step 7 — Windows サービス ✅ 実装完了（2026-08-15）
...
確認済み: テスト 21 件（設定 5 / DNS 差し替え 12 / ブラウザポリシー 4）。
ワークスペース全体で **247 件**。clippy 警告ゼロ。

**未確認**: サービスの登録・起動・停止・PC 再起動後の自動起動（管理者権限が要る）。
DNS 差し替えとブラウザポリシーは実装のみで、この PC には適用していない。
