...
│   │   ├── admin/              # 管理者ダッシュボード・ユーザー/ルーム管理・通報対応
│   │   ├── api/                # サーバーAPI（DM作成・管理者操作・プッシュ送信など）
...
│   │   ├── useUserSearch.ts    # ユーザー検索（名前・メールの部分一致、ブロック相手は除外）
...
rooms              トーク（グループ／DM。is_dm・dm_key でDMを表現）
...
user_blocks        ブロック関係（blocker → blocked）
reports            通報（メッセージ本文を保全・open/resolved）
...
|--------|----|----|
| id | UUID | トークID |
| name | TEXT | トーク名（**DMは空文字**で保存し、画面では相手の名前を動的表示） |
| created_by | UUID | 作成者のユーザーID |
| last_message_at | TIMESTAMPTZ | 最後のメッセージ日時 |
| is_dm | BOOLEAN | 1対1トーク（DM）フラグ |
| dm_key | TEXT | DM参加者2人のIDを昇順連結したキー。**部分ユニークインデックスで同じ相手とのDM二重作成をDBレベルで防止** |
...
| room_mutes | 全操作 | 本人のみ |
| user_blocks | 全操作 | **本人（ブロックした側）のみ**。誰にブロックされたかは見えない |
| reports | 作成 | 本人（通報者）のみ。**閲覧・対応は管理者API（service role）専用**（一般ユーザーは読めない） |
...
> **ブロックのDB強制**：DMへのメッセージ送信は `guard_dm_block` トリガー（BEFORE INSERT）が
> ブロック関係を**双方向**にチェックし、該当すればDBレベルで拒否します。
> クライアントを改造しても送信できません（新規DM作成もAPI側で拒否）。
...
- 管理者機能：ユーザー停止/解除・ルーム削除・メッセージ監視・**通報対応**（DMは「A ⇄ B」の参加者名で表示）

### トーク（ルーム）
- トーク一覧／作成／参加／退出／グループ名変更
- **1対1トーク（DM）**：新規トークの「1対1」タブで相手を選ぶと即開始。同じ相手とは既存トークを自動再利用（`dm_key` の一意制約で二重作成をDBレベル防止）。一覧・ヘッダー・通知には相手の名前とアバターを表示。DMに退出は無い（重複トーク防止）
- トーク一覧でグループのアバターに**人型アイコン**を表示（人名グループとDMを見分けられる）
- **グループのロール**（owner / admin / member）とメンバー管理（キック・ロール変更）
- **招待リンク**（7日有効）とユーザー検索（名前・メール）による直接追加
- **通知ミュート**（ルーム単位・ユーザーごと）

### ブロック・通報
- **ブロック**：DMのメニューから相手をブロック/解除。ブロック中は①新規DM開始不可 ②既存DMへの送信を**DBトリガーで双方向拒否** ③検索結果に表示されない ④入力欄が非表示になり理由バナーを表示
- **通報**：相手のメッセージを長押し→「通報」。本文を保全して記録し、管理者画面の「通報」タブで未対応/対応済みを管理
...
### ユーザー招待
- **名前またはメールアドレス**でユーザーを検索（部分一致・ブロック相手は非表示）
- トーク作成時にメンバーを招待
- 既存トークへのメンバー追加
...
- DMの通知タイトルは**送信者（相手）の名前**を表示（グループはトーク名）
